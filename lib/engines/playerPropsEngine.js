...
export class PlayerPropsEngine {
  ...
  async generateFeatures(input) {
    ...
    // ---- robust, crash-proof date handling ----
    let dateStr;
    try {
      ...
      if (this.apiClient && this.apiClient.apiKey) {
        ...
        if (sport === "NBA") {
          ...
          if (Array.isArray(stats)) {
            row = stats.find(s => String(s?.Name || "").toLowerCase()
  ...
  calculateStatisticalProbability(features, input) {
    ...
    const isMLB_K = String(input.sport || "").toUpperCase() === "MLB" && propText.includes("strikeout");

    const p = isMLB_K
      ? StatisticalModels.calculatePoissonProbability(mu, line)
      : StatisticalModels.calculateNormalProbability(mu, sigma, line);
  ...
  calibration(prob) { return prob * this.calibrationScale; }
  ...
  fuse(modelProb, marketProb, addOnNudges) {
    let fused =
      0.60 * modelProb +
      0.20 * marketProb +
      0.12 * 0.5 +     // placeholder sharp signal
      0.08 * 0.5 +
      addOnNudges;
    fused = this.calibration(clamp01(fused));
  ...
  async evaluateProp(inputRaw) {
    ...
    // never allow a thrown error to bubble up to the API
    let features;
    try { 
    ...
    const fused = this.fuse(stat.probability, market.marketProbability, nudgesTotal);
