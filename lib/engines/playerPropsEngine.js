...
// tolerant prop-normalizer for MLB strikeouts
function isMLBStrikeouts(prop){
  const p = String(prop||"");
  return /strike|strikout|strkeout|k(?![a-z])/i.test(p);
}
...
function _safeVariance(arr, minFloor=1.4){
  ...
  const xs = arr.map(n=>Number(n)||0);
  const mu = xs.reduce((a,b)=>a+b,0)/xs.length;
  const v  = xs.reduce((a,x)=>a+Math.pow(x-mu,2),0)/xs.length;
...
export class PlayerPropsEngine {
  constructor(apiClient) {
    ...
    this.recentSample = [];
    this.lastHttp = null; // capture from client
  ...
  validateInput(input) {
    this.errorFlags = [];
    for (const f of ["sport","player","prop"]) {
      if (!input || input[f] === undefined || input[f] === "") this.errorFlags.push(`MISSING_${f.toUpperCase()}`);
    }
  ...
  calculateExponentialAverage(arr, decay){
    ...
    let ws=0, tw=0;
    for (let i=0;i<arr.length;i++){ const w=Math.pow(decay,i); ws += (Number(arr[i])||0)*w; tw += w; }
    return tw>0 ? ws/tw : 0;
  }
  ...
  // fallback synthetic
  async getPlayerHistoricalStats(){ return {
  ...
  // pick stat field by sport/prop
  _pickValueFromRow(sport, prop, row) {
    const s = String(sport||"").toUpperCase();
    const p = String(prop||"").toLowerCase();

    if (s === "MLB") {
      if (isMLBStrikeouts(p)) return _mlbStrikeoutsFromRow(row);
      return NaN;
    }
    if (s === "NBA" || s === "WNBA") {
  ...
  _captureLastHttp(){
    if (this.apiClient && this.apiClient.lastHttp) this.lastHttp = this.apiClient.lastHttp;
  }
  ...
  async _byDateArray(sport, dateStr) {
    ...
    if (sport==="MLB" && typeof c.getMLBPlayerStatsByDate==="function"){
      ...
      const r = await c.getMLBPlayerStatsByDate(dateStr); this._captureLastHttp(); return r||[];
    }
    if (sport==="NBA" && typeof c.getNBAPlayerStatsByDate==="function"){
      this._pushUsed(`NBA:player-stats-by-date:${dateStr}`);
      const r = await c.getNBAPlayerStatsByDate(dateStr); this._captureLastHttp(); return r||[];
    }
    if (sport==="WNBA" && typeof c.getWNBAPlayerStatsByDate==="function"){
      this._pushUsed(`WNBA:player-stats-by-date:${dateStr}`);
      const r = await c.getWNBAPlayerStatsByDate(dateStr); this._captureLastHttp(); return r||[];
    }
    return [];
  }
  ...
  async _seasonArray(sport, season) {
    ...
    if (sport==="MLB" && typeof c.getMLBPlayerSeasonStats==="function"){
      ...
      const r = await c.getMLBPlayerSeasonStats(season); this._captureLastHttp(); return r||[];
    }
    if (sport==="NBA" && typeof c.getNBAPlayerSeasonStats==="function"){
      this._pushUsed(`NBA:player-season-stats:${season}`);
      const r = await c.getNBAPlayerSeasonStats(season); this._captureLastHttp(); return r||[];
    }
    if (sport==="WNBA" && typeof c.getWNBAPlayerSeasonStats==="function"){
      this._pushUsed(`WNBA:player-season-stats:${season}`);
      const r = await c.getWNBAPlayerSeasonStats(season); this._captureLastHttp(); return r||[];
    }
    if (sport==="NFL" && typeof c.getNFLPlayerSeasonStats==="function"){
      this._pushUsed(`NFL:player-season-stats:${season}`);
      const r = await c.getNFLPlayerSeasonStats(season); this._captureLastHttp(); return r||[];
    }
    return [];
  }
  ...
  async _nflWeekArray(season, week) {
    ...
    const r = await c.getNFLPlayerGameStatsByWeek(season, week); this._captureLastHttp(); return Array.isArray(r)?r:[];
  }
  ...
  async _collectRecentByDate(input, sport, startDateStr, lookbackDays, maxGames, idHint) {
    ...
    for (let d=0; d<lookbackDays && values.length<maxGames; d++){
      ...
      if (Array.isArray(arr) && arr.length){
        let row = null;
        if (idHint?.key && idHint?.value != null) row = arr.find(r => Number(r?.[idHint.key]) === Number(idHint.value));
        if (!row) row = arr.find(r => nameMatch(r?.Name));
        if (row){
          const v = this._pickValueFromRow(sport, input.prop, row);
          if (Number.isFinite(v)) values.push(v); else this.zeroFiltered++;
        }
      }
  ...
  // ---------- Feature builder (SDIO-first thorough; fallback only after attempts) ----------
  async generateFeatures(input) {
    ...
    try {
      if (this.apiClient) {
        // Attempt 1: by-date today/-1/-2 to lock an exact match & PlayerID
        const base = new Date(dateStr);
        const tries = [0,-1,-2].map(off => {
          const d = new Date(base); d.setDate(d.getDate()+off); return fmtLocalDate(d);
        });
        const nameMatch = _tokNameMatchFactory(input.player);
        let matched = null, idHint = null;

        for (const ds of tries) {
          const stats = await this._byDateArray(sport, ds);
          if (stats?.length) {
        ...
        // Attempt 2: season match (name or ID) to get season per-game
        let seasonAvg = null;
        const sArr = await this._seasonArray(sport, seasonYear);
        if (sArr?.length) {
          const sRow =
              (matched?.PlayerID ? sArr.find(r => Number(r?.PlayerID)===Number(matched.PlayerID)) : null)
           || sArr.find(r => nameMatch(r?.Name));
          if (sRow) {
            ...
            if (sport==="MLB") {
              ...
              if (gp>0) {
                const pp = String(input.prop||"").toLowerCase();
                if (pp.includes("rebound") && Number.isFinite(Number(sRow?.Rebounds))) seasonAvg = Number(sRow.Rebounds)/gp;
                else if (pp.includes("assist") && Number.isFinite(Number(sRow?.Assists))) seasonAvg = Number(sRow.Assists)/gp;
              }
            } else if (sport==="NFL") {
        ...
        // Attempt 3: build recents (NFL by week, others by date)
        let recentVals = [];
        if (matched || seasonAvg!=null) {
          if (sport==="NFL") {
            let season = seasonYear;
            try {
              const s = await this.apiClient.getNFLSeasonCurrent(); this._captureLastHttp();
              if (Number.isFinite(Number(s))) season = Number(s);
            } catch {}
            let week = null;
            try {
              const w = await this.apiClient.getNFLWeekCurrent(); this._captureLastHttp();
              if (Number.isFinite(Number(w))) week = Number(w);
            } catch {}
            if (!week) week = 18;
            recentVals = await this._collectNFLRecents(input, season, week, 8, matched?.PlayerID ? {key:"PlayerID", value:matched.PlayerID}:null);
          } else {
            recentVals = await this._collectRecentByDate(input, sport, dateStr, 60, 10, matched?.PlayerID ? {key:"PlayerID",value:matched.PlayerID}:null);
          }
        }
        ...
        // Choose mu
        const recentMean = recentVals.length>0 ? (recentVals.reduce((a,b)=>a+b,0)/recentVals.length) : (Number.isFinite(seasonAvg)?seasonAvg:NaN);
        let blendedMu = null;
        ...
        if (Number.isFinite(blendedMu)) {
          // Guard: MLB all-zeros recents => PASS
          if (sport==="MLB" && isMLBStrikeouts(input.prop) && recentVals.length>=5 && recentVals.every(x=>Number(x)===0)) {
            this.errorFlags.push("ZERO_RECENTS");
          }

          let variance;
          if (recentVals.length>=3) variance = this.calculateVariance(recentVals);
          else {
            if (sport==="MLB" && isMLBStrikeouts(input.prop)) variance = Math.max(1.44, Math.abs(blendedMu - blendedMu*0.9));
  ...
  calculateStatisticalProbability(features, input) {
    ...
    let mu = (Number(features.last30Avg)||0)
    ...
    if (sport==="MLB" && isMLBStrikeouts(propText)) {
  ...
  projectionGapNudge(modelProb, marketProb){
    ...
    if (gap >= this.thresholds.PROJECTION_GAP_TRIGGER) return 0.03*Math.sign(modelProb - marketProb);
    return 0;
  }
  ...
  applyHouseAdjustments(modelProb, input, features){
    ...
    if (stars.some(s=>String(input?.player||"").includes(s))) {
      adjusted -= this.thresholds.NAME_INFLATION; flags.push("NAME_INFLATION");
    }
    ...
    if (isHalf) {
      ...
      if (Math.abs((features?.last30Avg||0) - line) < 0.3) {
        adjusted -= this.thresholds.HOOK_BUFFER; flags.push("HOOK_TRAP");
      }
    }

    if ((features?.stdDev||0) > 4 && (String(input.sport).toUpperCase()!=="NFL")) {
      adjusted -= this.thresholds.VARIANCE_PENALTY; flags.push("HIGH_VARIANCE");
    }

    // Guard: pathological combo (tiny σ & huge μ) → dampen
    if ((String(input.sport).toUpperCase()==="MLB" && isMLBStrikeouts(input.prop))
        && (features?.stdDev||0) < 1.2 && (features?.last30Avg||0) > 7.5) {
      adjusted = Math.min(adjusted, 0.60); flags.push("DAMPED_OVERCONFIDENCE");
    }

    return { adjustedProb: clamp01(adjusted), flags };
  }

  applyCalibration(p){ return p * this.calibrationFactor; }
  ...
  // Safety Gate: only allow LEAN+ if we truly used SDIO and have signal
  _safetyDecision(fused, features){
    const used = Array.isArray(this.usedEndpoints) ? this.usedEndpoints : [];
    const usedSdio = (this.dataSource==="sportsdata" && used.length>0);
    const hasSample = (this.recentValsCount>=3) || Number.isFinite(features?.last30Avg);

    // force PASS when not grounded in SDIO data
    if (!usedSdio || !hasSample || this.errorFlags.includes("ZERO_RECENTS")) {
      return { decision: "PASS", finalConfidence: Math.min(49.9, Math.round(fused*1000)/10) };
    }
    // normal thresholds
    const conf = Math.round(fused*1000)/10;
    ...
    const decision =
      conf >= LOCK ? "LOCK" :
      conf >= SLEAN ? "STRONG_LEAN" :
      conf >= LEAN ? "LEAN" : "PASS";
    return { decision, finalConfidence: conf };
  }

  // ---------- main ----------
  async evaluateProp(inputRaw) {
    const input = {
      sport: inputRaw?.sport || "",
      player: inputRaw?.player || "",
      opponent: inputRaw?.opponent || "",
      prop: inputRaw?.prop || "",
      odds: {
        over: Number(inputRaw?.odds?.over) || Number(inputRaw?.over) || NaN,
        under: Number(inputRaw?.odds?.under) || Number(inputRaw?.under) || NaN,
      },
      startTime: inputRaw?.startTime || inputRaw?.date || null,
    ...
    catch { features = { last30Avg:0, variance:1, stdDev:1, matchupFactor:1, minutesFactor:1, specific:{adjustment:0} }; }
    ...
    const { decision, finalConfidence } = this._safetyDecision(fused, features);

    const suggestion = (stat.probability>=0.5) ? "OVER" : "UNDER";

    const DBG = {};
    if (this.apiClient && this.apiClient.lastHttp) DBG.lastHttp = this.apiClient.lastHttp;
    if (!this.apiClient || !this.apiClient.apiKey) DBG.fallbackReason = "NO_API_KEY";
    ...
    return {
      ...
      flags: [
        ...this.errorFlags,
        ...houseFlags,
        (this.dataSource==="sportsdata" && this.usedEndpoints.length>0) ? "SDIO_OK" : "FALLBACK_DATA",
        (SMART ? "SMART_OVERLAYS" : "SMART_OFF"),
        (!((this.dataSource==="sportsdata") && this.usedEndpoints.length>0) ? "INSUFFICIENT_SAMPLE" : null)
      ].filter(Boolean),
      rawNumbers: {
