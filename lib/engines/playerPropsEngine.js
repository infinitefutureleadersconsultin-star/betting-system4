...
export class PlayerPropsEngine {
  ...
  // Pull a numeric stat by checking multiple common field names (projections + stats)
  getFirstNumber(row, keys) {
    for (const k of keys) {
      const v = row?.[k];
  ...
  // Map prop text to likely fields across sports
  pickStatFromProp(prop, row) {
    const p = String(prop || "").toLowerCase();
    if (p.includes("assist"))     return this.getFirstNumber(row, ["Assists","ProjectedAssists"]);
    if (p.includes("rebound"))    return this.getFirstNumber(row, ["Rebounds","TotalRebounds","ProjectedRebounds"]);
    if (p.includes("point"))      return this.getFirstNumber(row, ["Points","ProjectedPoints"]);
    if (p.includes("three") || p.includes("3pt")) return this.getFirstNumber(row, ["ThreePointersMade","ProjectedThreePointersMade"]);
    if (p.includes("steal"))      return this.getFirstNumber(row, ["Steals","ProjectedSteals"]);
    if (p.includes("block"))      return this.getFirstNumber(row, ["BlockedShots","Blocks","ProjectedBlocks"]);
    if (p.includes("turnover"))   return this.getFirstNumber(row, ["Turnovers","ProjectedTurnovers"]);
    if (p.includes("strikeout"))  return this.getFirstNumber(row, ["PitchingStrikeouts","StrikeoutsPitched","ProjectedStrikeouts","Strikeouts"]);
    if (p.includes("hits"))       return this.getFirstNumber(row, ["Hits","ProjectedHits"]);
    if (p.includes("total bases"))return this.getFirstNumber(row, ["TotalBases","ProjectedTotalBases"]);
    if (p.includes("passing"))    return this.getFirstNumber(row, ["PassingYards","ProjectedPassingYards"]);
    if (p.includes("rushing"))    return this.getFirstNumber(row, ["RushingYards","ProjectedRushingYards"]);
    if (p.includes("receiving"))  return this.getFirstNumber(row, ["ReceivingYards","ProjectedReceivingYards"]);
    return 0;
  }

  // ---------- Feature builder (SportsDataIO first → safe fallback) ----------
    ...
    try {
      if (this.apiClient && this.apiClient.apiKey) {
        ...
        for (const dStr of datesToTry) {
          ...
          if (sport === "MLB") {
            // 1) Projections first
            list = await this.apiClient.getMLBPlayerProjectionsByDate(dStr);
            if (Array.isArray(list)) {
              this.usedEndpoints.push(`MLB:projections:${dStr}`);
              row = list.find(r => nameMatches(r?.Name || r?.PlayerName || r?.Player));
              matchedName = row?.Name || row?.PlayerName || null;
            }
            // 2) Stats as fallback seed
            if (!row) {
              list = await this.apiClient.getMLBPlayerStats(dStr);
              if (Array.isArray(list)) {
                this.usedEndpoints.push(`MLB:stats:${dStr}`);
                row = list.find(r => nameMatches(r?.Name || r?.PlayerName || r?.Player));
                matchedName = row?.Name || row?.PlayerName || null;
              }
            }
            // 3) Optional: Starting lineups (only to boost minutesFactor if present)
            if (row) {
              const lineups = await this.apiClient.getMLBStartingLineups(dStr);
              if (Array.isArray(lineups)) this.usedEndpoints.push(`MLB:lineups:${dStr}`);
            }

          } else if (sport === "NBA") {
            // 1) Projections first
            list = await this.apiClient.getNBAPlayerProjectionsByDate(dStr);
            if (Array.isArray(list)) {
              this.usedEndpoints.push(`NBA:projections:${dStr}`);
              row = list.find(r => nameMatches(r?.Name || r?.PlayerName || r?.Player));
              matchedName = row?.Name || row?.PlayerName || null;
            }
            // 2) Stats fallback seed (same-day)
            if (!row) {
              list = await this.apiClient.getNBAPlayerStats(dStr);
              if (Array.isArray(list)) {
                this.usedEndpoints.push(`NBA:stats:${dStr}`);
                row = list.find(r => nameMatches(r?.Name || r?.PlayerName || r?.Player));
                matchedName = row?.Name || row?.PlayerName || null;
              }
            }
            // 3) Optional: schedules/odds (not strictly used yet; safe context)
            const _games = await this.apiClient.getNBAGamesByDate(dStr);
            if (Array.isArray(_games)) this.usedEndpoints.push(`NBA:games:${dStr}`);

          } else if (sport === "WNBA") {
            // (Projections often limited; stats are the safer call)
            list = await this.apiClient.getWNBAPlayerStats(dStr);
            if (Array.isArray(list)) {
              this.usedEndpoints.push(`WNBA:stats:${dStr}`);
              row = list.find(r => nameMatches(r?.Name || r?.PlayerName || r?.Player));
              matchedName = row?.Name || row?.PlayerName || null;
            }
            const _games = await this.apiClient.getWNBAGamesByDate(dStr);
            if (Array.isArray(_games)) this.usedEndpoints.push(`WNBA:games:${dStr}`);
          }
