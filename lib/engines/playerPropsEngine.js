...
export class PlayerPropsEngine {
  constructor(apiClient) {
    ...
    // numeric factor must NOT collide with a method name
  ...
  async generateFeatures(input) {
    ...
    try {
      if (this.apiClient && this.apiClient.apiKey) {
        // Try today, then -1, -2 days to increase hit rate when games have not posted yet
        ...
        for (const dStr of datesToTry) {
          ...
          if (sport === "NBA") {
            ...
            stats = await this.apiClient.getWNBAPlayerStats(); // no date param
          } else if (sport === "MLB") {
        ...
        // if we get here, no row found; keep trying fallback below
        this._usedEndpoints = tried;
      }
