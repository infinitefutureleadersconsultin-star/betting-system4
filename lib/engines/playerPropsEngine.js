...
export class PlayerPropsEngine {
  constructor(apiClient) {
    this.apiClient = apiClient || null;   // may be empty; we fallback gracefully
    this.errorFlags = [];
    ...
    this.calibrationScale = 1.0; // tune later with calibration runs
  }
  ...
  async getPlayerHistoricalStats() {
    // Fallback synthetic history (keeps the engine running without live data)
    return {
  ...
  // ---------- data assembly ----------
  // ✅ Robust date parsing added here; never throws even with bad input
  async generateFeatures(input) {
    ...
    try {
      // Try a tiny live pull from SportsDataIO if we have a key
          ...
          if (Array.isArray(stats)) {
            row = stats.find(s =>
              String(s?.Name || "").toLowerCase()
                .includes(String(input.player || "").split(" ")[0].toLowerCase())
            );
          }
        } else if (sport === "WNBA") {
      ...
      // Fallback (no live row or no key)
      ...
      // Absolute safety valve: never throw out of this function
      const playerStats = await this.getPlayerHistoricalStats(input.player, sport);
      features.last30Avg = this.calculateExponentialAverage(playerStats.last30, 0.90);
      features.last7Avg  = this.calculateExponentialAverage(playerStats.last7, 0.85);
      features.variance  = this.calculateVariance(playerStats.recent);
  ...
  // ---------- SMART overlays (tiny bounded nudges) ----------
  ...
  workloadGuardrail(input, features) {
    ...
    if (!w || w.includes("low") || w.includes("?")) {
      return -0.03;
    }
  ...
  steamDetectionNudge() {
    if (!SMART) return 0;
    // No line-move history yet → neutral. (Wire up later when available.)
    return 0;
  }

  calibrationScale(prob) {
    // After collecting outcomes, tune this scaler for reliability
    return prob * this.calibrationScale;
  }
  ...
  fuseProbabilities(modelProb, marketProb, sharpSignal, addOnNudges) {
    const base =
      0.60 * modelProb +
      0.20 * marketProb +
      0.12 * (0.5 + (Number(sharpSignal) || 0)) + // placeholder
      0.08 * 0.5;

    let fused = base + addOnNudges;
    fused = this.calibrationScale(clamp01(fused));
  ...
  async evaluateProp(inputRaw) {
    ...
    // ✅ Absolute safety: even if generateFeatures throws, keep going
    let features;
    try {
    ...
    const fused = this.fuseProbabilities(stat.probability, market.marketProbability, 0 /*sharp*/, nudgesTotal);
