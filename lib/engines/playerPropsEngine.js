...
export class PlayerPropsEngine {
  ...
  _pickValueFromRow(sport, prop, row) {
    ...
    if (s === "MLB") {
      if (p.includes("strikeout")) {
        const k1 = Number(row.PitcherStrikeouts);
        if (Number.isFinite(k1)) return k1;
        const k2 = Number(row.StrikeoutsPitched);
        if (Number.isFinite(k2)) return k2;
        const k3 = Number(row.Strikeouts);
        if (Number.isFinite(k3)) return k3;
        return NaN;
      }
      return NaN;
    }
  ...
  _pickSeasonPerGame(sport, prop, row) {
    ...
    if (s === "MLB" && p.includes("strikeout")) {
      ...
      const ip = Number(row.InningsPitched) || Number(row.InningsPitchedDecimal);
      ...
      if (Number.isFinite(k9)) {
        const proxyIP = Number(row.InningsPitched) || Number(row.InningsPitchedDecimal) || (Number(row.Games) || 0) * 1.0;
        const games = Number(row.GamesStarted) || Number(row.Games) || 0;
  ...
  _isPitchedAppearance(row) {
    const ip = Number(row.InningsPitched) || Number(row.InningsPitchedDecimal) || 0;
    const outs = Number(row.PitchingOuts) || 0;
    const batters = Number(row.BattersFaced) || Number(row.PitchingBattersFaced) || 0;
    const gs = Number(row.GamesStarted) || 0;
    const gp = Number(row.GamesPitched) || Number(row.Appearances) || 0;
    return (ip > 0) || (outs > 0) || (batters > 0) || (gs > 0) || (gp > 0);
  }
    ...
    if (s === "MLB" || s === "NBA" || s === "WNBA") {
      ...
      for (let i = 0; i < _windowDays && vals.length < maxGames; i++) {
        ...
        if (s === "MLB") {
          ...
          // Require a pitching appearance; drop 0s from non-pitch days
        ...
        if (v === 0) this.zeroFiltered += 1; // true 0 while pitching (rare) — keep it
        vals.push(v);
      }
      return vals;
    }
  ...
  async evaluateProp(inputRaw) {
    ...
    return {
      ...
      meta: {
        ...
        zeroFiltered: this.zeroFiltered,      // true 0-K games kept (rare); zeros from non-pitch days are excluded entirely
