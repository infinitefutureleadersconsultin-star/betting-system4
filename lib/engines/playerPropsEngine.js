...
export class PlayerPropsEngine {
  constructor(apiClient) {
    ...
    this.calibrationFactor = 1.0; // tune via calibration later
  }
  ...
  // more tolerant field mapping across *stats* and *projections*
  pickStatFromProp(prop, row) {
    const p = String(prop || "").toLowerCase();

    // MLB pitching strikeouts can be named differently across endpoints
    const any = (...keys) => {
      for (const k of keys) {
        const v = row?.[k];
    ...
    if (p.includes("strikeout")) {
      // try the common projection & stats field names
      const k = any("StrikeoutsPitched", "PitcherStrikeouts", "Strikeouts");
      return k || 0;
    }
  ...
  // ---------- Feature builder (tries projections first, then stats; falls back safely) ----------
  async generateFeatures(input) {
    ...
    try {
      ...
      dateStr = fmtLocalDate(d); // local (not UTC)
    } catch {
    ...
    try {
      if (this.apiClient && this.apiClient.apiKey) {
        // Try today, then -1 day (stats are often final for yesterday; projections for today)
        ...
        const tokens = String(input.player || "")
          .toLowerCase()
          .split(/\s+/)
          .filter(Boolean);
        const nameMatches = (candidate) => {
          const cand = String(candidate || "").toLowerCase();
          return tokens.some(tok => cand.includes(tok));
        };
        ...
        for (const dStr of datesToTry) {
          ...
          if (sport === "MLB") {
            // 1) projections by date (works before the game starts)
            tried.push(`MLB:player-projections-by-date:${dStr}`);
            list = await this.apiClient.getMLBPlayerProjections(dStr);
            if (Array.isArray(list)) {
              row = list.find(s => nameMatches(s?.Name));
              if (row) { this.dataSource = "sportsdata"; break; }
            }

            // 2) final stats by date (yesterday tends to work)
            tried.push(`MLB:player-stats-by-date:${dStr}`);
            ...
            tried.push(`NBA:player-projections-by-date:${dStr}`);
            list = await this.apiClient.getNBAPlayerProjections(dStr);
            if (Array.isArray(list)) {
              row = list.find(s => nameMatches(s?.Name));
              if (row) { this.dataSource = "sportsdata"; break; }
            }

            tried.push(`NBA:player-stats-by-date:${dStr}`);
            ...
            // no reliable per-date projections; use season stats (no date)
            tried.push(`WNBA:player-season-stats`);
            list = await this.apiClient.getWNBAPlayerStats();
            if (Array.isArray(list)) {
              row = list.find(s => nameMatches(s?.Name));
              if (row) { this.dataSource = "sportsdata"; break; }
            }
