...
export class PlayerPropsEngine {
  ...
  makeNameMatcher(targetFull) {
    ...
    return (candidate) => {
      ...
      // prefer last-name presence; fall back to any token
      return c.includes(` ${last} `) || c.endsWith(` ${last}`) || c.startsWith(`${last} `) || tokens.some(t => c.includes(` ${t} `) || c.startsWith(`${t} `) || c.endsWith(` ${t}`));
    };
  }

  // tolerant field mapping across stats & projections
  ...
  async generateFeatures(input) {
    ...
    try {
      if (this.apiClient && this.apiClient.apiKey) {
        ...
        // try today and -1 day for per-date endpoints
        ...
        // If still no row, try season stats to at least get a live-backed mean
        if (!row) {
          if (sport === "MLB") {
            const season = new Date(dateStr).getFullYear();
            tried.push(`MLB:player-season-stats:${season}`);
            const list = await this.apiClient.getMLBPlayerSeasonStats(season);
            if (Array.isArray(list)) {
              const r = list.find(s => match(s?.Name));
              if (r) {
                // crude per-appearance average for Ks
                const k = this.pickStatFromProp(input.prop, r); // should see Strikeouts*
                const apps = Number(r?.GamesStarted || r?.Games || r?.Appearances || 0) || 1;
                const statGuess = (Number(k) || 0) / apps;
            ...
            const season = new Date(dateStr).getFullYear();
            tried.push(`NBA:player-season-stats:${season}`);
            const list = await this.apiClient.getNBAPlayerSeasonStats(season);
            if (Array.isArray(list)) {
              const r = list.find(s => match(s?.Name));
              if (r) {
                // simple per-game stat guess for points/assists/rebounds
                const p = this.pickStatFromProp(input.prop, r);
                const g = Number(r?.Games || r?.GamesPlayed || 0) || 1;
                const statGuess = (Number(p) || 0) / g;
                const variance  = Math.max(0.5, Math.abs(statGuess - statGuess * 0.9));
                this._usedEndpoints = tried;
                this.dataSource = "sportsdata";
                return {
