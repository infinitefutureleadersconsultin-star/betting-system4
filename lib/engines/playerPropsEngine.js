...
export class PlayerPropsEngine {
  ...
  _pickValueFromRow(sport, prop, row) {
    ...
    if (s === "MLB") {
      // By-date (game) pitching Ks are on PitcherStrikeouts (sometimes StrikeoutsPitched)
  ...
  _pickSeasonPerGame(sport, prop, row) {
    ...
    if (s === "MLB" && p.includes("strikeout")) {
      ...
      if (Number.isFinite(k9) && Number.isFinite(ip) && gs > 0) {
        const ipPerStart = ip / gs; // can be <5 or >6 based on role
        const perStart = (k9 * ipPerStart) / 9;
        if (Number.isFinite(perStart) && perStart > 0) return perStart;
      }

      // 3) As last resort, if we only have K/9 and some proxy for usage
      if (Number.isFinite(k9)) {
        const proxyIP = Number(row.InningsPitched) || (Number(row.Games) || 0) * 1.0; // 1.0 IP proxy if nothing else
