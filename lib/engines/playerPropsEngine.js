...
export class PlayerPropsEngine {
  ...
  _isPitchingRowMLB(row) {
    ...
    const pos = String(row.PositionCategory || row.Position || "").toUpperCase();
    if (pos.startsWith("P")) return true;
    if (Number(row.InningsPitchedDecimal) > 0) return true;
    // presence of pitching-only fields also indicates pitcher row
    if ("StrikeoutsPitched" in row || "PitchingStrikeouts" in row || "PitcherStrikeouts" in row) return true;
    return false;
  }
  ...
  _pitcherKsFromRow_STRICT(row) {
    // 1) direct integer-like strikeout fields (best)
    const direct = [
      row?.StrikeoutsPitched,
      row?.PitchingStrikeouts,
      row?.PitcherStrikeouts,
      // Some feeds may just use "Strikeouts" on pitcher rows
      row?.Strikeouts
    ];
    for (const v of direct) {
      const n = Number(v);
      if (Number.isFinite(n)) {
        // counting stat; force integer
        return this._roundCount(n);
      }
    }

    // 2) derive from K/9 and innings pitched if available
    const k9 = Number(row?.PitchingStrikeoutsPerNineInnings ?? row?.StrikeoutsPerNineInnings);
    const ip = Number(row?.InningsPitchedDecimal);
    if (Number.isFinite(k9) && Number.isFinite(ip)) {
      // strikeouts ≈ K/9 * IP / 9
      const est = (k9 * ip) / 9;
      return this._roundCount(est);
    }

    // 3) last resort: null (skip this row)
    return null;
  }

  // ---------- NBA/WNBA helpers ----------
  _extractNBAWNBAStat(row, statKind) {
    // counting stats; coerce to integer
    if (statKind === "rebounds") {
      const candidates = [row?.Rebounds, row?.TotalRebounds, row?.ReboundsTotal];
      for (const v of candidates) {
        const r = this._roundCount(v);
        if (r !== null) return r;
      }
    }
    if (statKind === "assists") {
  ...
  _qbPassingYardsFromRow(row) {
    ...
    for (const v of candidates) {
      const n = Number(v);
      if (Number.isFinite(n)) return Math.round(Math.max(0, n));
    }
    return null;
  }
        ...
        if (this.apiClient && typeof this.apiClient.getMLBPlayerStats === "function") {
          ...
          if (Array.isArray(stats) && stats.length) {
            ...
            for (const r of rows) {
              ...
              const k = this._pitcherKsFromRow_STRICT(r);
              if (k == null) continue;
              out.push(k); // integer Ks only
              if (!this.matchedName) this.matchedName = String(r.Name || "");
              break; // one game per day
            }
          }
        }
      }
    ...
    try {
      ...
      for (let i = 0; i < dayWindow && out.length < maxGames; i++) {
        ...
        if (Array.isArray(stats) && stats.length) {
          ...
          if (v == null) continue;
          out.push(v); // integer rebounds/assists
  ...
  async generateFeatures(input) {
      ...
      try {
        ...
        if (this.apiClient?.getMLBPlayerSeasonStats) {
          ...
          if (Array.isArray(seasonData) && seasonData.length) {
            ...
            if (row) {
              const ksTotal = Number(
                row.PitchingStrikeouts ?? row.PitcherStrikeouts ?? row.StrikeoutsPitched ?? row.Strikeouts
              );
              const starts = Number(row.GamesStarted ?? row.GamesPitched ?? row.Games ?? 0);
              if (Number.isFinite(ksTotal) && starts > 0) {
                seasonAvg = ksTotal / starts;
                if (!this.matchedName) this.matchedName = String(row.Name || "");
              }
            }
          }
        ...
        const recentMean = recentVals.length > 0
          ...
          : (seasonAvg ?? baseline);
        const blendedMu = (seasonAvg != null) ? 0.6 * recentMean + 0.4 * seasonAvg : recentMean;

        // variance from sample if possible; otherwise a conservative floor
        let variance =
          (recentVals.length >= 3) ? this.calculateVariance(recentVals)
                                   : Math.max(1.44, Math.abs(blendedMu - blendedMu * 0.9)); // >= 1.2^2 floor
        let sigma = Math.sqrt(variance);
        // MLB Ks sanity limits
    ...
    if ((sport === "NBA" || sport === "WNBA") && (propText.includes("rebound") || propText.includes("assist"))) {
      try {
        ...
        const baseline = statKind === "rebounds" ? 6 : 4;
        const recentMean = recentVals.length > 0
          ? recentVals.reduce((a, b) => a + b, 0) / recentVals.length
          : (seasonAvg ?? baseline);
        const blendedMu = (seasonAvg != null) ? 0.6 * recentMean + 0.4 * seasonAvg : recentMean;
        ...
        let variance = (recentVals.length >= 3)
          ...
          : Math.max(1.44, Math.abs(blendedMu - blendedMu * 0.85)); // >= 1.2^2
        let sigma = Math.sqrt(variance);
    ...
    if (sport === "NFL" && (propText.includes("pass") || propText.includes("passing"))) {
      try {
        const recentVals = await this._collectRecentPassingYdsNFL(dateStr, this._nameTokens(input.player), 8);
        this.recentValsCount = recentVals.length;
        ...
        if (this.apiClient?.getNFLPlayerSeasonStats) {
          ...
          if (Array.isArray(seasonData) && seasonData.length) {
            const row = seasonData.find(r => this._candidateMatches(this._nameTokens(input.player), r?.Name) && this._isQBRowNFL(r));
            if (row) {
        ...
        let sigma = Math.sqrt(variance);
        sigma = Math.max(15, Math.min(sigma, 120)); // sanity for passing yards
        variance = sigma * sigma;
