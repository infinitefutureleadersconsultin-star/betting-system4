...
export class PlayerPropsEngine {
  ...
  _pickValueFromRow(sport, prop, row) {
    ...
    if (s === "MLB") {
      // By-date (game) pitching Ks are on PitcherStrikeouts (sometimes StrikeoutsPitched/Strikeouts)
  ...
  // -------- season per-game value (uses *PerGame if present; else totals/Games; MLB has K/9 fallback) --------
  _pickSeasonPerGame(sport, prop, row) {
    const s = String(sport || "").toUpperCase();
    const p = String(prop || "").toLowerCase();

    if (s === "MLB" && p.includes("strikeout")) {
      // 1) Direct totals preferred
      let tot =
      ...
      // 2) Derive per-start via K/9 × IP per start
      const k9 =
        Number(row.StrikeoutsPerNine) ||
        Number(row.PitcherStrikeoutsPerNine);
      const ip = Number(row.InningsPitched);
      ...
      // 3) As last resort, crude proxy if only K/9 + Games present
      if (Number.isFinite(k9)) {
        const proxyIP = Number(row.InningsPitched) || (Number(row.Games) || 0) * 1.0;
  ...
  async _collectRecentGamesForRollingMean(input, sport, dateStr, windowDays = 45, maxGames = 10) {
    ...
    // MLB: widen to 120 days to ensure we pick up last ~10 starts even with off-days
  ...
  calculateStatisticalProbability(features, input) {
    ...
    // MLB strikeouts → Poisson; NBA/WNBA/NFL → Normal
    ...
    if (sport === "MLB" && propText.includes("strikeout")) {
      ...
      if (sport === "NFL" && propText.includes("passing") && propText.includes("yard")) {
        sigma = Math.max(15, Math.min(sigma, 90)); // gentle NFL volatility band
      }
