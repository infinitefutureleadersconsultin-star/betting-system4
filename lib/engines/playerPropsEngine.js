...
export class PlayerPropsEngine {
  ...
  // Pitching row detector for MLB (by-date rows)
  _isPitchingRow(row) {
    const ip = Number(row?.InningsPitched ?? row?.PitchingInningsPitched ?? row?.PitchingInnings ?? 0);
    return Number.isFinite(ip) && ip > 0;
  }

  // Extract the metric from a row given the prop & sport
  pickStatFromProp(prop, row, sport, isSeasonTotals = false) {
    ...
    if (sport === "MLB" && p.includes("strikeout")) {
      if (isSeasonTotals) {
        const totalK =
          Number(row?.PitchingStrikeouts ?? row?.StrikeoutsPitched ?? row?.Strikeouts ?? 0);
        const starts =
          Number(row?.GamesStarted ?? row?.Games ?? row?.Appearances ?? 0);
        if (totalK > 0 && starts > 0) return totalK / starts;
        return null; // no usable season signal
      } else {
        if (!this._isPitchingRow(row)) return null; // skip non-pitching rows
        const k = Number(row?.PitchingStrikeouts ?? row?.StrikeoutsPitched ?? row?.Strikeouts ?? 0);
        return Number.isFinite(k) ? k : null;
      }
    }

    // Generic hoops props (NBA/WNBA)
    if (p.includes("assist"))  return Number(row?.Assists ?? row?.AssistsPercentage ?? 0) || null;
    if (p.includes("rebound")) return Number(row?.Rebounds ?? row?.TotalRebounds ?? 0) || null;
    if (p.includes("point"))   return Number(row?.Points ?? 0) || null;

    // NFL example (expand later)
    if (p.includes("passing")) return Number(row?.PassingYards ?? 0) || null;

    return null;
  }

  // ---------- Feature builder (SportsDataIO → Season → Synthetic) ----------
  async generateFeatures(input) {
    ...
    // 1) BY-DATE GAME STATS (last ~4 days sweep) → require correct row type (e.g., pitcher)
    try {
      if (this.apiClient && this.apiClient.apiKey) {
        ...
        for (const dStr of datesToTry) {
          ...
          const raw = this.pickStatFromProp(input.prop, row, sport, /*season*/ false);
          const val = acceptValue(raw);
          if (val != null) {
    ...
    // 2) SEASON-TO-DATE (FREE TIER) → per-start/per-appearance K; enrich with recent by-date (pitching rows only)
    try {
      if (this.apiClient && this.apiClient.apiKey) {
        let season = null;
        ...
        if (Array.isArray(season) && season.length > 0) {
          ...
          if (row) {
            const raw = this.pickStatFromProp(input.prop, row, sport, /*season*/ true);
            const seasonAvg = acceptValue(raw);
            if (seasonAvg != null) {
              this.matchedName = String(row.Name || "");

              // Recent by-date (only pitching rows, last 30d, up to 10)
              ...
              const variance = (recentVals.length >= 3)
                ...
                : Math.max(0.5, Math.abs(last30Avg - last30Avg * 0.9));

              this.dataSource = "sportsdata";
              return {
                last60Avg: last30Avg,
                last30Avg,
                last7Avg:  recentVals.length > 0 ? this.calculateExponentialAverage(recentVals.slice(0, 7), 0.85) : last30Avg,
                variance,
                stdDev: Math.sqrt(variance),
                matchupFactor: 1.0,
                minutesFactor: 1.0,
  ...
  // Only count pitching games for MLB K when building recent rolling sample
  async _collectRecentGamesForRollingMean(input, sport, anchorDateStr, lookbackDays = 30, takeN = 10) {
    ...
    for (let off = -1; off >= -lookbackDays; off--) {
      ...
      // For MLB strikeouts, require a pitching row
      const propText = String(input.prop || "").toLowerCase();
      if (sport === "MLB" && propText.includes("strikeout") && !this._isPitchingRow(row)) continue;
