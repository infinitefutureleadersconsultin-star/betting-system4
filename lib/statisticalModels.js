...
export class StatisticalModels {
  ...
  static normalCDF(x, mean, stdDev) {
    if (!Number.isFinite(x) || !Number.isFinite(mean) || !Number.isFinite(stdDev) || stdDev <= 0) return 0.5;
  ...
  static calculatePoissonProbability(lambda, line) {
    ...
    const k = Math.ceil(Number(line) || 0) - 1;  // P(X > line)
  ...
  static calculateNormalProbability(mean, stdDev, line) {
    ...
    return 1 - this.normalCDF((Number(line) || 0) + 0.5, mean, stdDev); // continuity correction
  }
}
